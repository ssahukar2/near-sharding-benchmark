#!/usr/bin/env bash
# RPS sweep for sharded-bm (genesis create-accounts path). requests_per_second = per shard.
set -u

# Auto-detect RPC port when not already set (e.g. run_sweep_* already exported RPC_ADDR).
if [ -z "${RPC_ADDR:-}" ]; then
...
wait_for_rpc() {
    ...
    while true; do
        # Do not let curl nonzero exit kill the script under set -e.
        code="000"
...
RPSS=(1000 2000 3000 5000 7500 9000)
TOTAL=${#RPSS[@]}
...
for i in "${!RPSS[@]}"; do
  ...
  echo ""
  echo "========== Run ${RUN}/${TOTAL} | RPS (per shard) ${RPS} =========="

  jq --argjson r "${RPS}" '.requests_per_second = $r' "${PARAMS}" > /tmp/params.$$.json && mv /tmp/params.$$.json "${PARAMS}"

  echo "[Run ${RUN}/${TOTAL} | RPS ${RPS}] → init..."
  ./bench.sh init "${CASE}" || { echo "init failed"; exit 1; }

  echo "[Run ${RUN}/${TOTAL} | RPS ${RPS}] → create-accounts..."
  ./bench.sh create-accounts "${CASE}" || { echo "create-accounts failed"; exit 1; }

  echo "[Run ${RUN}/${TOTAL} | RPS ${RPS}] → start-nodes..."
  ./bench.sh start-nodes "${CASE}" || { echo "start-nodes failed"; exit 1; }

  wait_for_rpc "http://${RPC_ADDR}" || exit 1

  rm -f "${HOME}/iostat_${RPS}.txt"
  iostat -x 1 120 > "${HOME}/iostat_${RPS}.txt" &
  IOP=$!

  echo "[Run ${RUN}/${TOTAL} | RPS ${RPS}] → native-transfers..."
  ERRF=$(mktemp)
  t0=$(date +%s)
  set +e
...
echo ""
echo "Sweep finished. Wrote ${RESULT_CSV}"
cat "${RESULT_CSV}"
